...
AgentPlane is an AI Gateway — a middleware layer between AI applications and LLM providers. It authenticates callers via API keys, runs configurable guardrails on input and output, injects the upstream provider key, forwards to an OpenAI-compatible `chat/completions` endpoint, and returns the response.

Built in Go 1.26.5 with PostgreSQL for user/API-key persistence, a pluggable secrets layer (env, file, Vault, AWS, Azure), and an extensible guardrail pipeline.
